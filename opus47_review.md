...
- **`Logic/ExpressionSolver/ExpressionExtensions.cs:16-24` — `IsToken` calls `Enum.GetValues<Token>().Any(...)` per character.** Reflection per char per expression. Cache as `static readonly HashSet<char>`.
...
- **`Actors/Logic/LogicUnit.Hit.cs:60-81` — `GetHitChanceForTargetNumber`** lacks an explicit case for `targetNumber == 2`; falls through to `default: return 1m` (correct but undocumented). `> 12` returns `0m`, `< 3` returns `1m` — inconsistent style.
- **`Actors/Logic/LogicUnit.Damage.cs:10` — `using Microsoft.CodeAnalysis;`** unused import.
